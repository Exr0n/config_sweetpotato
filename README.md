...
Binds beyond the old skhd: `⌥⇧i` pop-to-fresh-workspace · `⌥i` **gather mode** (see below) · `⌥u` recently-created (via `$AEROSPACE_WINDOW_ID` catch-all rule) · `⌥⇧v` golden-ratio center-float · `⌥⇧a` **toggle light/dark** (osascript appearance) · `⌥q` Todoist · `⌥o` Claude · `⌥;` ChatGPT-Codex (physical cmd-;) · `⌥⇧o` timestamp-paste script · `⌃⌥⇧hjkl` **join-with** (nested containers; then `⌥b`/`⌥⇧b` set the sub-layout)
...
- **Expanded curated set:** `macos-defaults.sh` (repo root) is the hand-picked, re-runnable settings block — keyboard/text, trackpad+mouse speed, tap-to-click, Dock (incl. `mru-spaces=false` for AeroSpace), Finder (column view, search-current-folder, desktop drives), analog clock, Stage Manager off, Accessibility Zoom (ctrl-scroll), text replacements. Curated by **reading every domain** in `archive/2026-06-01-config/CONFIG/macos_settings/key_defaults_readable.txt` (19 domains) — NOT a blind import (e.g. old `screencapture show-thumbnail=0` was deliberately overridden to keep the floating thumbnail). Judgment calls (re-add Chinese language, auto light/dark, menu-bar layout) are left commented under `# REVIEW`. The **full old-machine dump is captured** there; when a setting seems "missing," mine that file rather than re-deriving.
...
7. **"Can't switch away from app X after undocking"** (its full-screen window stays drawn on top; other/tagged spaces switch fine): X's window got **orphaned on a disconnected monitor's Space**, so AeroSpace can't see it — check `aerospace list-windows --workspace <x>` (empty despite the app running) and `... --all | grep X` (absent). `reload-config` and even a full AeroSpace restart **won't** recover it (the window's on a phantom Space). Fix: **quit & reopen app X** so it recreates its window on the live Space → AeroSpace re-detects it. Seen with Discord after external monitors disconnect.
